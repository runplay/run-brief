...
public class OAuth2SaslClient implements SaslClient {
    ...
    public OAuth2SaslClient(String oauthToken,
        ...
        //BLog.e("IN","OAuth2SaslClient.init: "+oauthToken);
    ...
    public byte[] evaluateChallenge(byte[] challenge) throws SaslException {
        //BLog.e("IN","OAuth2SaslClient.eval challenge");
        ...
        byte[] response=null;
        if(token!=null) {
            response = String.format("user=%s\1auth=Bearer %s\1\1", email, token).getBytes();
        } else {
            response = String.format("user=%s\1auth=Bearer %s\1\1", email, oauthToken).getBytes();
        }
