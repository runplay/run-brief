...
public class GatewayDevice {
    ...
    public String getExternalIPAddress() {
        ...
        return Gateway.IP_NONE;
    }
    private static final int MAX_LOAD_ALL_DEVICES=10;
    public ArrayList<PortMappingEntry> getAllPortMappings() {
        ArrayList<PortMappingEntry> ports=new ArrayList<PortMappingEntry>();
        for(int i=0; i<MAX_LOAD_ALL_DEVICES; i++) {
            PortMappingEntry pme=null;
            try {
                pme=getGenericPortMappingEntry(i);
            } catch (WeUPnPException wex) {}
            if(pme!=null)
                ports.add(pme);
            else
        ...
        BLog.e("GTD", "Got "+ports.size()+" portmappings");
        return ports;
    }
...
BLog.e("add portmapping ec: "+nameValue.get("errorCode"));
