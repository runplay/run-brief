...
public class Sf {
   ...
   public static String javascriptSafe(String text) {
	...
	public static ArrayList<Person> stripdownEmails(String emaillist) {
		ArrayList<Person> stripemails=new ArrayList<Person>();
		if(emaillist!=null) {
			String spitter=",";
			if(emaillist.indexOf(";")!=-1)
				spitter=",";
			String [] split = emaillist.split(spitter);
			if(split!=null) {
				for(int i=0; i<split.length; i++) {
					...
					if(spl.indexOf("<")!=-1) {
						...
						spl=sp1[1].substring(0,sp1[1].indexOf(">"));
						} catch(Exception e) {}
					}
						
					spl=spl.trim();
				    
					if(Sf.isValidEmail(spl)) {
						//BLog.e("GCONTACT", "get with email: "+spl);
						Person p = ContactsDb.getWithEmail(spl);
						if(p==null) {
							p=new Person();
							p.setString(Person.STRING_NAME,spl);
							p.setString(Person.STRING_PERSON_ID, Person.generateNoId());
                            JSONArray email=new JSONArray().put(spl);
							p.setJSONArray(Person.JSONARRAY_EMAIL,email);
